...
public static class PerehidnykPOPP
{
	...
	public static double[,] MaterialPerehidnykPOPP(int B, int H, int B1, int H1, int L, int k, int k1, int markaStali, double tovchynaMetalu, int typPOPP, int typProfilB1H1, int typProfilB2H2, int number, int polkaB1H1, int polkaB2H2, int vidbortovka, string aisiType = "430")
	{
		...
		double num11 = (double)(num3 + 10) * (num9 + (double)k + (double)k1);
		if (num3 > 2500)
		{
			num11 += 60.0 * num9;
		}
		...
		if ((typPOPP == 1 || typPOPP == 2) && typProfilB1H1 == 0)
		{
			num3 = ((B >= H) ? B : H);
			typProfilB1H1 = ((num3 < 800) ? 1 : 2);
		}
		if ((typPOPP == 1 || typPOPP == 3) && typProfilB2H2 == 0)
		{
			num3 = ((B1 >= H1) ? B1 : H1);
	...
	public static double PricePerehidnykPOPP(int B, int H, int B1, int H1, int L, int k, int k1, int markaStali, double tovchynaMetalu, int typPOPP, int vidbortovka)
	{
		double num = 1.0;
		int num2 = ((B >= H) ? B : H);
		L += vidbortovka;
		...
		case 1:
			...
			num = ((typPOPP > 3) ? ((!(tovchynaMetalu < 0.65)) ? ((tovchynaMetalu < 0.9) ? 406.0 : 520.0) : 345.0) : ((!(tovchynaMetalu < 0.65)) ? ((tovchynaMetalu < 0.9) ? 464.0 : 600.0) : 402.5));
			break;
		case 2:
		{
			if (tovchynaMetalu == 0.0)
	...
	public static double VolumePerehidnykPOPP(int B, int H, int B1, int H1, int L, int k, int k1)
	{
		int num = ((B > B1) ? B : B1);
		int num2 = ((H > H1) ? H : H1);
		int num3 = L + k + k1;
