...
public static class PerehidnykPOP
{
	...
	public static double[,] MaterialPerehidnykPOP(int B, int H, int B1, int H1, int L, int k, int k1, int markaStali, double tovchynaMetalu, int typPOP, int typProfilB1H1, int typProfilB2H2, int number, int polkaB1H1, int polkaB2H2, int vidbortovka, string aisiType = "430")
	{
		...
		array[0, 0] = (num10 + 2.0 * num12 + num11) * (1.0 + num2 / 100.0) / 1000000.0;
		num10 = (double)(B + B1 + 20) * (double)L / 2.0 + (double)((B + 10) * k) + (double)((B1 + 10) * k1);
		num10 += 2.0 * ((double)(H + H1 + 100) * Math.Sqrt(4.0 * (double)L * (double)L + (double)((B1 - B) * (B1 - B))) / 4.0 + (double)((H + 50) * k) + (double)((H1 + 50) * k1));
		...
		if ((typPOP == 1 || typPOP == 2) && typProfilB1H1 == 0)
		{
			num3 = ((B >= H) ? B : H);
			typProfilB1H1 = ((num3 < 800) ? 1 : 2);
		}
		if ((typPOP == 1 || typPOP == 3) && typProfilB2H2 == 0)
		{
			num3 = ((B1 >= H1) ? B1 : H1);
	...
	public static double PricePerehidnykPOP(int B, int H, int B1, int H1, int L, int k, int k1, int markaStali, double tovchynaMetalu, int typPOP, int vidbortovka)
	{
		double num = 1.0;
		int num2 = ((B >= H) ? B : H);
		L += vidbortovka;
		...
		case 1:
			...
			num = ((typPOP > 3) ? ((!(tovchynaMetalu < 0.65)) ? ((tovchynaMetalu < 0.9) ? 406.0 : 520.0) : 345.0) : ((!(tovchynaMetalu < 0.65)) ? ((tovchynaMetalu < 0.9) ? 464.0 : 600.0) : 402.5));
			break;
		case 2:
		{
			if (tovchynaMetalu == 0.0)
		...
		materialSpendingCommercialOffer += 2.0 * ((double)(H + H1) * Math.Sqrt(4.0 * (double)L * (double)L + (double)((B1 - B) * (B1 - B))) / 4.0 + (double)(H * k) + (double)(H1 * k1));
	...
	public static double VolumePerehidnykPOP(int B, int H, int B1, int H1, int L, int k, int k1)
	{
		int num = ((B > B1) ? B : B1);
		int num2 = ((H > H1) ? H : H1);
		int num3 = L + k + k1;
