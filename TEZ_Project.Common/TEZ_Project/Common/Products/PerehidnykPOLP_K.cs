...
public static class PerehidnykPOLP_K
{
	...
	public static double[,] MaterialPerehignykPOLP_K(int B, int H, int D, int L, int k, int k1, int markaStali, double tovchynaMetalu, int typPOLP_K, int typProfilBH, int typProfilD, int number, int polkaBH, int vidbortovka, string aisiType = "430")
	{
		...
		num3 = ((H > D) ? H : D);
		num3 += 50.0;
		num12 = Math.Sqrt((double)L * (double)L + (num10 - (double)B) * (num10 - (double)B)) + (double)k + (double)k1;
		double num16 = num3 * num12;
		if (num3 > 2500.0)
		{
			num16 += 60.0 * num12;
		}
		...
		array[0, 0] = (num13 + num14 + num15 + num16) * (1.0 + num2 / 100.0) / 1000000.0;
		num13 = ((double)B + num10) * (double)L / 2.0 + (double)(B * k) + num10 * (double)k1;
		num13 += ((double)H + num11) * (double)L / 2.0 + (double)(H * k) + num11 * (double)k1;
		num13 += ((double)B + num10) * Math.Sqrt((double)L * (double)L + (num11 - (double)H) * (num11 - (double)H)) / 2.0 + (double)(B * k) + num10 * (double)k1;
		num13 += ((double)H + num11) * Math.Sqrt((double)L * (double)L + (num10 - (double)B) * (num10 - (double)B)) / 2.0 + (double)(H * k) + num11 * (double)k1;
		num13 /= 1000000.0;
		...
		int num17 = 1;
		int num18 = 0;
		if (typPOLP_K == 1 || typPOLP_K == 2 || typPOLP_K == 9)
		{
			if (typProfilBH == 0)
			{
				num3 = ((B >= H) ? B : H);
			...
			else
			{
				if (FlanetsPrjamokutnyj.materialName[0] != "")
				{
					...
					for (int l = 0; l < 6; l++)
					{
						array[num17, l] = ((l == 1) ? array2[0, l] : (array[num17, l] + array2[0, l]));
					}
				}
				num17++;
			}
			...
			for (int m = 0; m < 6; m++)
			{
				array[num17, m] = array3[0, m];
			}
			num17++;
		}
		if (typPOLP_K == 1 || typPOLP_K == 3 || typPOLP_K == 10)
		{
			...
			for (int n = 0; n < 5; n++)
			{
				if (FlanetsKruglyj.materialName1[n] == materialName[num18])
				{
					for (int num19 = 0; num19 < 6; num19++)
					{
						array[num18, num19] += array4[n, num19];
					}
					array[num18, 1] = Math.Round(100.0 * array[num18, 1] / 2.0) / 100.0;
				}
				else
				{
					...
					{
						array[n + num17, num20] = array4[n, num20];
					}
				}
			}
		}
	...
	public static double PricePerehidnykPOLP_K(int B, int H, int D, int L, int k, int k1, int markaStali, double tovchynaMetalu, int typPOLP_K, int typProfilD, int vidbortovka)
	{
		double num = 1.0;
		int num2 = ((B >= H) ? B : H);
		L += vidbortovka;
	...
	public static double VolumePerehidnykPOLP_K(int B, int H, int D, int L, int k, int k1)
	{
		double num = ((B > D) ? B : D);
		double num2 = ((H > D) ? H : D);
		double num3 = L + k + k1;
