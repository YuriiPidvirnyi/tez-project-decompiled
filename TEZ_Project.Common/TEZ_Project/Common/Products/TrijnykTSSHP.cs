...
public static class TrijnykTSSHP
{
	...
	public static double[,] MaterialTrijnykTSSHP(int B, int H, int B1, int k1, int k2, int markaStali, double tovchynaMetalu, int typTSSHPenter, int typTSSHPout, int typProfilBH, int typProfilB1H1, int number, int polkaBH, int polkaB1H1, string aisiType = "430")
	{
		...
		double num9 = (double)B + (double)(2 * k2);
		double num10 = num9 * (double)(H + 50);
		if (num9 > 1250.0)
		{
			num10 += 30.0 * (double)(H + 50);
		}
		double num11 = 2.0 * (double)(k1 + k2) * (double)(H + 50);
		num9 = 2.0 * (double)k2 + (double)B + 10.0;
		double num12 = num9 * (double)(B1 + k1);
		if (num9 > 2500.0)
		{
			num12 += 60.0 * (double)(B1 + k1);
		}
		else if (num9 > 1250.0)
		{
			num12 += 30.0 * (double)(B1 + k1);
		}
		if (B1 + k1 > 2500)
		{
			num12 += 60.0 * num9;
		...
		num10 = (B + 2 * k2) * (H + 50 + 10);
		num11 = 2 * (k1 + k2) * (H + 50 + 10);
		num12 = (B + 2 * k2) * B1 + k1 * B;
		num2 = Math.Round((1000000.0 * array[0, 0] / (num10 + num11 + 2.0 * num12) - 1.0) * 100.0);
		if (num2 > 20.0)
		{
			num2 = 20.0;
			array[0, 0] = (num10 + num11 + 2.0 * num12) * (1.0 + num2 / 100.0) / 1000000.0;
		...
		if (typTSSHPenter == 1 && typProfilBH == 0)
		{
			num3 = ((B >= H) ? B : H);
			typProfilBH = ((num3 < 800) ? 1 : 2);
		}
		if (typTSSHPout == 1 && typProfilB1H1 == 0)
		{
			num3 = ((B1 >= H) ? B1 : H);
	...
	public static double PriceTrijnykTSSHP(int B, int H, int B1, int k1, int k2, int markaStali, double tovchynaMetalu, int typTSSHPenter, int typTSSHPout)
	{
		double num = 1.0;
		int num2 = ((B >= H) ? B : H);
		if (B1 >= num2)
		...
		case 1:
			...
			num = ((typTSSHPenter != 1 && typTSSHPout != 1) ? ((!(tovchynaMetalu < 0.65)) ? ((tovchynaMetalu < 0.9) ? 406.0 : 520.0) : 345.0) : ((!(tovchynaMetalu < 0.65)) ? ((tovchynaMetalu < 0.9) ? 464.0 : 600.0) : 402.5));
			break;
		case 2:
		{
			if (tovchynaMetalu == 0.0)
		...
		double num7 = (B + 2 * k2) * H;
		double num8 = 2 * (k1 + k2) * H;
		double num9 = (B + 2 * k2) * B1 + k1 * B;
		materialSpendingCommercialOffer = (num7 + num8 + 2.0 * num9) / 1000000.0;
	...
	public static double VolumeTrijnykTSSHP(int B, int H, int B1, int k1, int k2)
	{
		return Math.Round(((double)B1 + (double)k1) * (double)H * (double)(B + 2 * k2) / 1000000.0) / 1000.0;
	}
}
