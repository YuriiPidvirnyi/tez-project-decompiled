...
public static class TrijnykTSHSP
{
	...
	public static double[,] MaterialTrijnykTSHSP(int B, int H, int B1, int k1, int k2, int r, int markaStali, double tovchynaMetalu, int typTSHSPenter, int typTSHSPout, int typProfilBH, int typProfilB1H1, int number, int polkaBH, int polkaB1H1, string aisiType = "430")
	{
		...
		double num9 = (double)B + (double)(2 * (r + k2));
		double num10 = num9 * (double)(H + 50);
		if (num9 > 1250.0)
		{
			num10 += 30.0 * (double)(H + 50);
		}
		double num11 = 2.0 * (Math.PI * (double)r / 2.0 + (double)k1 + (double)k2) * (double)(H + 50);
		double num12 = ((double)B + (double)(2 * (r + k2))) * (double)(B1 + k1 + r + 10);
		num9 = (double)B + (double)(2 * (r + k2));
		if (num9 > 2500.0)
		{
			num12 += 60.0 * (double)(B1 + k1 + r + 10);
		}
		else if (num9 > 1250.0)
		{
			num12 += 30.0 * (double)(B1 + k1 + r + 10);
		}
		num9 = B1 + k1 + r + 10;
		num12 = ((!(num9 > 2500.0)) ? (num12 + 30.0 * (double)(B + 2 * (r + k2))) : (num12 + 60.0 * (double)(B + 2 * (r + k2))));
		array[0, 0] = (num10 + num11 + 2.0 * num12) * (1.0 + num2 / 100.0) / 1000000.0;
		num10 = (B + 2 * (r + k2)) * H;
		num11 = 2.0 * (Math.PI * (double)r / 2.0 + (double)k1 + (double)k2) * (double)H;
		num12 = 2.0 * (double)B * (double)(B1 + k1 + r) + (double)(4 * B1 * (r + k2)) + 0.8584073464102069 * (double)r * (double)r;
		num2 = Math.Round((1000000.0 * array[0, 0] / (num10 + num11 + num12) - 1.0) * 100.0);
		if (num2 > 20.0)
		{
			num2 = 20.0;
			array[0, 0] = (num10 + num11 + num12) * (1.0 + num2 / 100.0) / 1000000.0;
		}
		...
		if (typTSHSPenter == 1 && typProfilBH == 0)
		{
			num3 = ((B >= H) ? B : H);
			typProfilBH = ((num3 < 800) ? 1 : 2);
		}
		if (typTSHSPout == 1 && typProfilB1H1 == 0)
		{
			num3 = ((B1 >= H) ? B1 : H);
		...
		if (typProfilBH == 1 || typProfilB1H1 == 1)
		{
			...
			if (typProfilB1H1 == 1)
			{
				array2 = FlanetsPrjamokutnyj.MaterialFlanetsPrjamokutnyj(B1, H, typProfilB1H1, number, polkaB1H1, 2);
				materialCost += FlanetsPrjamokutnyj.materialCost1;
				for (int l = 0; l < 2; l++)
				{
					if (FlanetsPrjamokutnyj.materialName[l] != "")
					{
						materialName[num13 + l] = FlanetsPrjamokutnyj.materialName[l];
						for (int m = 0; m < 6; m++)
						{
							array[num13 + l, m] = ((m == 1) ? array2[l, m] : (array[num13 + l, m] + array2[l, m]));
						}
					}
				}
			}
		...
		if (typProfilB1H1 == 1 || typProfilB1H1 == 2)
		{
			...
			{
				array[num13, num18] += 2.0 * array4[0, num18];
			}
		}
	...
	public static double PriceTrijnykTSHSP(int B, int H, int B1, int k1, int k2, int r, int markaStali, double tovchynaMetalu, int typTSHSPenter, int typTSHSPout)
	{
		double num = 1.0;
		int num2 = ((B >= H) ? B : H);
		if (B1 >= num2)
		...
		case 1:
			...
			num = ((typTSHSPenter != 1 && typTSHSPout != 1) ? ((!(tovchynaMetalu < 0.65)) ? ((tovchynaMetalu < 0.9) ? 406.0 : 520.0) : 345.0) : ((!(tovchynaMetalu < 0.65)) ? ((tovchynaMetalu < 0.9) ? 464.0 : 600.0) : 402.5));
			break;
		case 2:
		{
			if (tovchynaMetalu == 0.0)
		...
		double num7 = (B + 2 * (r + k2)) * H;
		double num8 = 2.0 * (Math.PI * (double)r / 2.0 + (double)k1 + (double)k2) * (double)H;
		double num9 = 2.0 * (double)B * (double)(B1 + k1 + r) + 4.0 * (double)B1 * (double)(r + k2) + 0.8584073464102069 * (double)r * (double)r;
		materialSpendingCommercialOffer = (num7 + num8 + num9) / 1000000.0;
	...
	public static double VolumeTrijnykTSHSP(int B, int H, int B1, int k1, int k2, int r)
	{
		return Math.Round(((double)B1 + (double)r + (double)k1) * (double)H * (double)(B + 2 * (k2 + r)) / 1000000.0) / 1000.0;
	}
}
