...
public static class UtkaURNP
{
	...
	public static double[,] MaterialUtkaURNP(int B, int H, int H1, int L, int C, int k, int k1, int markaStali, double tovchynaMetalu, int typURNP, int typProfilBH, int typProfilB1H1, int number, int polkaBH, int polkaB1H1, int vidbortovka, string aisiType = "430")
	{
		...
		L -= k + k1;
		double num9 = (double)B * ((double)L - Math.Sqrt((double)L * (double)L - (double)(2 * B * C) + (double)(C * C))) / (double)(2 * B - C);
		double num10 = Math.Sqrt(((double)L - num9) * ((double)L - num9) + (double)(C * C));
		double num11 = ((double)B + 10.0) * (2.0 * (double)(k + k1) + (num9 + num10) * Math.Sqrt(4 * L * L + (H - H1) * (H - H1)) / (double)L);
		double num12 = B + C + 10;
		if (num12 > 2500.0)
		{
			num11 += 120.0 * (double)(L + k + k1);
		}
		if (num12 > 1250.0)
		{
			num11 += 60.0 * (double)(L + k + k1);
		}
		if (L + k + k1 > 2500)
		{
			num11 += 120.0 * num12;
		...
		double num13 = 2.0 * (double)((H + 50) * k + (H1 + 50) * k1) + (double)(H + H1 + 100) * (num9 + num10);
		num3 = ((H > H1) ? H : H1);
		num12 = num9 + num10 + (double)k + (double)k1;
		if (num12 > 2500.0)
		{
			num13 += 120.0 * (double)(num3 + 50);
		}
		else if (num12 > 1250.0)
		{
			num13 += 60.0 * (double)(num3 + 50);
		}
		if (num3 + 50 > 2500)
		{
			num13 += 120.0 * num12;
		}
		else if (num3 + 50 > 1250)
		{
			num13 += 60.0 * num12;
		}
		array[0, 0] = (num11 + num13) * (1.0 + num2 / 100.0) / 1000000.0;
		num11 = 2.0 * (double)((B + 10 + H + 50) * k + (B + 10 + H1 + 50) * k1) + (double)(H + H1 + 100) * (num9 + num10);
		num11 += (double)(B + 10) * (num9 + num10) * Math.Sqrt(4.0 * (double)L * (double)L + (double)((H - H1) * (H - H1))) / (double)L;
		num11 /= 1000000.0;
		...
		if ((typURNP == 1 || typURNP == 2) && typProfilBH == 0)
		{
			num3 = ((B >= H) ? B : H);
			typProfilBH = ((num3 < 800) ? 1 : 2);
		}
		if (typURNP == 1 && typProfilB1H1 == 0)
		{
			num3 = ((B >= H1) ? B : H1);
		...
		if (typProfilBH == 1 || typProfilB1H1 == 1)
		{
			...
			if (typProfilB1H1 == 1)
			{
				array2 = FlanetsPrjamokutnyj.MaterialFlanetsPrjamokutnyj(B, H1, typProfilB1H1, number, polkaB1H1, 1);
				materialCost += FlanetsPrjamokutnyj.materialCost1;
				for (int l = 0; l < 2; l++)
				{
	...
	public static double PriceUtkaURNP(int B, int H, int H1, int L, int C, int k, int k1, int markaStali, double tovchynaMetalu, int typURNP, int vidbortovka)
	{
		double num = 1.0;
		int num2 = ((B >= H) ? B : H);
		L += vidbortovka;
		...
		case 1:
			...
			num = ((typURNP > 2) ? ((!(tovchynaMetalu < 0.65)) ? ((tovchynaMetalu < 0.9) ? 406.0 : 520.0) : 345.0) : ((!(tovchynaMetalu < 0.65)) ? ((tovchynaMetalu < 0.9) ? 464.0 : 600.0) : 402.5));
			break;
		case 2:
		{
			if (tovchynaMetalu == 0.0)
		...
		materialSpendingCommercialOffer = 2.0 * (double)((B + H) * k + (B + H1) * k1) + (double)(H + H1) * (num7 + num8);
		materialSpendingCommercialOffer += (double)B * (num7 + num8) * Math.Sqrt(4.0 * (double)L * (double)L + (double)((H - H1) * (H - H1))) / (double)L;
	...
	public static double VolumeUtkaURNP(int B, int H, int H1, int L, int C)
	{
		int num = ((H > H1) ? H : H1);
		return Math.Round((double)num * (double)(B + C) * (double)L / 1000000.0) / 1000.0;
	}
}
